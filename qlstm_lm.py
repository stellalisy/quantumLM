...
class Q_LSTM_Model_LM(nn.Module):
    def __init__(self, device:str, vocab_size:int, max_grad:float, embed_dims:int, num_layers:int,
        ...
        lstms = [qlstm.QLSTM(input_size=embed_dims, hidden_size=embed_dims, batch_first=True)
    ...
    def forward(self, x:Tensor, states:List[Tuple[Tensor, Tensor]]) -> Tuple[Tensor, List[Tuple[Tensor, Tensor]]]:
        ...
        for i, lstm in enumerate(self.lstms):
            #if len(list(states[i][0].shape)) == 2:
                #states[i][0].unsqueeze_(0)
            x, states[i] = lstm(x, states[i])
            #print('states',states[i][0].shape)
            x = self.dropout(x)
        scores = self.fc(x)
